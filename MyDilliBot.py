    ...
    while True:
        ...
        if(updates.get("result")):
            if len(updates["result"]) > 0:
                ...
                if (updates["result"][last_update].get("message")):
                    if (updates["result"][last_update]["message"]["chat"].get("id")):
                        chatId = updates["result"][last_update]["message"]["chat"]["id"]
                        if str(chatId) == "-" or str(chatId) == "-" or str(chatId) == "-":

                            if (updates["result"][last_update]["message"].get("text")):
